...
async fn main() -> Result<()> {
    // 启动Anvil，创建Wallet, 创建http链接
    let http_anvil_provider = ProviderBuilder::new().on_anvil_with_wallet();

    // Deploy the contract
    let counter_contract = CounterWithError::deploy(http_anvil_provider.clone()).await?;

    // setup filter
    let increment_filter = counter_contract.Increment_filter().watch().await?;
    let decrement_filter = counter_contract.Decrement_filter().watch().await?;

    // convert filter to stream
    let mut increment_stream = increment_filter.into_stream();
    let mut decrement_stream = decrement_filter.into_stream();

    // create contract call
    let increment_call = counter_contract.increment();
    let decrement_call = counter_contract.decrement();

    // wait
    let _increnemt_res = increment_call.send().await?;
    ...
    let err_call = counter_contract.revertA();
    ...
    let err_call = counter_contract.revertB();
    let err_result = err_call.send().await;
    if let Err(err) = err_result {
        println!("Error B: {err:#?}");
    }
