...
impl<P: Provider> Deployer<P> {
    fn new(provider: P, wallet: EthereumWallet) -> Self {
...
async fn main() -> Result<()> {
    ...
    let wallet = EthereumWallet::new(signer_pk);

    let deployer = Deployer::new(provider.clone(), wallet);
