...
async fn main() -> Result<()> {
    ...
    for i in 0..length.to::<u64>() {
        ...
        // 计算第i个元素第一个slot的位置
        let slot_i = lock_info_slot_data + (index * U256::from(2));

        let slot_i_1_data = http_provider
            .get_storage_at(*es_rnt_address, slot_i)
            .await?;
        ...
        // 第二个slot的位置
        let slot2 = slot_i + U256::from(1);
        let slot2_data = http_provider.get_storage_at(*es_rnt_address, slot2).await?;

        let slot2_data_bytes: [u8; 32] = slot2_data.to_be_bytes_vec().try_into().expect("failed");
        let amount = U256::from_be_bytes(slot2_data_bytes);
